...
void OBCameraNode::getParameters() {
  ...
  setAndGetNodeParameter(parameters_, rotate_image_180_, "rotate_image_180", false);
}
...
void OBCameraNode::onNewFrameCallback(const openni::VideoFrameRef& frame,
  ...
  cv::Mat scaled_image;
  const cv::Mat* publish_mat = &image;
  if (stream_index == DEPTH) {
    cv::resize(image, scaled_image, cv::Size(width * depth_scale_, height * depth_scale_), 0, 0,
               cv::INTER_NEAREST);
    publish_mat = &scaled_image;
  }
  cv::Mat rotated_image;
  if (rotate_image_180_) {
    cv::flip(*publish_mat, rotated_image, -1);
    publish_mat = &rotated_image;
  }
  auto image_msg =
      cv_bridge::CvImage(std_msgs::msg::Header(), encoding_.at(stream_index), *publish_mat)
          .toImageMsg();
  ...
  image_msg->width = publish_mat->cols;
  image_msg->height = publish_mat->rows;
